...
var Tooltip_default = BdApi.Components.Tooltip;
...
function filterDeclarations(module2, filter) {
  for (const key in module2.declarations) {
    const declaration = module2.declarations[key];
    if (filter(declaration)) return declaration;
  }
  return null;
}

// plugins/ImageOCR/config.json
var config_default = {
...
async function getWorker() {
  if (!worker) {
    worker = await (0, import_tesseract.createWorker)();
  }
  return worker;
}
